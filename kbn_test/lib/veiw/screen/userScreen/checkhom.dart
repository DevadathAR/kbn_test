...
import 'package:kbn_test/veiw/screen/userScreen/userWidgets/checkfilter.dart';
...
class _UserHomeState extends State<UserHome> {
  ...
  void initState() {
    ...
    _fetchJobTitles();
    _checkResumeLink(); 
    // Check if the resume link exists when initializing
  }
  ...
  Future<List<dynamic>> _fetchFilteredJobs(int pageNumber) async {
    try {
      ...
      setState(() {
        _jobs = filteredJobs;
        int totalJobs =
            response['totaljobs'] ?? 0; // Default to 0 if not present
