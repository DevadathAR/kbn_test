...
Apiresponse welcomeFromJson(String str) =>
...
class CompanyData {
  ...
  factory CompanyData.fromJson(Map<String, dynamic> json) => CompanyData(
        ...
        jobsPageData: List<JobsPageDatum>.from(
            json["jobsPageData"].map((x) => JobsPageDatum.fromJson(x))),
        messagesPageData: List<MessagesPageDatum>.from(
            json["messagesPageData"].map((x) => MessagesPageDatum.fromJson(x))),
      );
  ...
  Map<String, dynamic> toJson() => {
        ...
        "messagesPageData":
            List<dynamic>.from(messagesPageData.map((x) => x.toJson())),
      };
}
...
class ApplicantsPageData {
  ...
  factory ApplicantsPageData.fromJson(Map<String, dynamic> json) =>
      ApplicantsPageData(
        pending:
            List<Pending>.from(json["pending"].map((x) => Pending.fromJson(x))),
        selected: List<dynamic>.from(json["selected"].map((x) => x)),
      );

  Map<String, dynamic> toJson() => {
        "pending": List<dynamic>.from(pending.map((x) => x.toJson())),
        "selected": List<dynamic>.from(selected.map((x) => x)),
      };
}
...
enum ResumeLink { UPLOADS_DEVADATH_AR_PDF, UPLOADS_RECTANGLE_740_PNG }
...
final statusValues = EnumValues({"submitted": Status.SUBMITTED});
...
class MessagesPageDatum {
  ...
  factory MessagesPageDatum.fromJson(Map<String, dynamic> json) =>
      MessagesPageDatum(
...
class StatisticsPageData {
  ...
  factory StatisticsPageData.fromJson(Map<String, dynamic> json) =>
      StatisticsPageData(
        recruitment: List<Recruitment>.from(
            json["recruitment"].map((x) => Recruitment.fromJson(x))),
      );
